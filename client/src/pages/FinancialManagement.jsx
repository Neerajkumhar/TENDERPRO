...
const FinancialManagement = ({ onInvoiceClick }) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      {/* Header */}
      ...
      <div className="grid grid-cols-12 gap-4 sm:gap-8">
        ...
        <div className="col-span-12 card p-4 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 relative overflow-hidden">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-slate-500 font-medium">Monthly financial comparison</p>
            </div>
            <div className="flex items-center gap-4 sm:gap-6">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 sm:w-3 sm:h-3 bg-blue-500 rounded-full shadow-lg shadow-blue-200"></span>
                <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">Revenue</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 sm:w-3 sm:h-3 bg-slate-300 rounded-full"></span>
                <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">Expense</span>
              </div>
            </div>
          </div>
        ...
        <div className=\"col-span-12 lg:col-span-4\">
          <div className=\"card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 h-full\">
          ...
          <div className=\"card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 h-full\">
            ...
            <div className=\"h-[200px] sm:h-[250px] relative\">
              ...
              <div className=\"absolute inset-0 flex items-center justify-center flex-col\">
                <span className=\"text-xl sm:text-2xl font-black text-slate-900\">₹8.5L</span>
                <span className=\"text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase\">Total</span>
              </div>
            </div>
        ...
        <div className=\"col-span-12 lg:col-span-4\">
          <div className=\"card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 h-full\">
            ...
            <div className=\"space-y-4\">
              {[1, 2, 3].map((_, i) => (
                <div key={i} className=\"p-3 sm:p-4 rounded-2xl bg-slate-50 border border-slate-100 group hover:border-blue-200 transition-all\">
                  <div className=\"flex justify-between items-start\">
                    <div>
                      <p className=\"text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest\">Upcoming payment {i + 1}</p>
                      <p className=\"text-[11px] sm:text-xs font-bold text-slate-700 mt-1\">Acme - 11, 2024</p>
                    </div>
                    <p className=\"text-xs sm:text-sm font-black text-slate-900\">₹15,000</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className=\"col-span-12 card bg-white border-none shadow-xl shadow-slate-200/40 overflow-hidden\">
          <div className=\"p-4 sm:p-8 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50/30 gap-4\">
            <div>
              <h3 className=\"font-black text-slate-900 text-lg sm:text-xl tracking-tight\">Invoice Status Table</h3>
              <p className=\"text-[10px] sm:text-xs text-slate-500 font-medium mt-1\">Detailed log of recent financial transactions</p>
            </div>
            <div className=\"flex items-center gap-3 w-full sm:w-auto\">
              ...
              <button 
                ...
                className=\"flex-1 sm:flex-none bg-slate-900 text-white px-6 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-slate-200 hover:bg-blue-600 transition-all uppercase tracking-widest\"
              >
                Generate Invoice
              </button>
            </div>
            ...
            <table className=\"w-full text-left\">
              <thead>
                <tr className=\"bg-slate-50/50 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]\">
                  <th className=\"px-6 sm:px-8 py-4\">ID</th>
                  <th className=\"px-6 sm:px-8 py-4\">Date</th>
                  <th className=\"px-6 sm:px-8 py-4\">Client</th>
                  <th className=\"px-6 sm:px-8 py-4 text-center\">Amount</th>
              ...
              <tbody className=\"divide-y divide-slate-50\">
                {invoicesList.map((inv, i) => (
                  <tr key={i} className=\"hover:bg-blue-50/30 transition-all group\">
                    <td className=\"px-6 sm:px-8 py-4 sm:py-5 text-[10px] sm:text-xs font-bold text-slate-400\">{inv.invoiceNumber || inv.id}</td>
                    <td className=\"px-6 sm:px-8 py-4 sm:py-5 text-[10px] sm:text-xs font-bold text-slate-600\">
                      {inv.date ? new Date(inv.date).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: '2-digit' }) : 'N/A'}
                    </td>
                    <td className=\"px-6 sm:px-8 py-4 sm:py-5 text-xs sm:text-sm font-black text-slate-800\">{inv.client}</td>
                    <td className=\"px-6 sm:px-8 py-4 sm:py-5 text-xs sm:text-sm font-black text-slate-900 text-center\">
                    ...
                    <td className=\"px-6 sm:px-8 py-4 sm:py-5\">
                      <div className={`w-fit px-2 sm:px-3 py-1 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-widest shadow-sm
          ...
          <div className=\"bg-white w-full max-w-xl rounded-3xl sm:rounded-[2.5rem] shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 flex flex-col max-h-[90vh]\">
            <div className=\"p-6 sm:p-10 overflow-y-auto custom-scrollbar\">
              <div className=\"flex justify-between items-center mb-8\">
                <div>
                  <h2 className=\"text-xl sm:text-2xl font-black text-slate-900 tracking-tighter italic uppercase\">Generate New Invoice</h2>
