              ...
              <div className="flex bg-slate-50 p-1 rounded-xl overflow-x-auto">
                {['Year', 'Month', 'Week', 'Day'].map((v) => (
                  <button
                    key={v}
                    onClick={() => setView(v)}
                    className={`px-2 md:px-4 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black transition-all ${view === v ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {v}
                  </button>
                ))}
                    ...
                    {upcomingEvents.length === 0 ? (
                      ...
                      upcomingEvents.map(e => (
                        <div key={e.id} className="flex justify-between items-center text-[10px] font-bold text-slate-500 gap-2">
                          ...
                          <span className="text-slate-400 shrink-0 font-medium" style={{ color: e.color }}>{e.date.toLocaleDateString()}</span>
                        </div>
                      ))
                    )}
                  </div>
