...
const CalendarPage = () => {
    ...
    try {
      ...
      tenders.forEach(t => {
        if (t.submissionDate) {
          allEvents.push({
            id: `t_${t.id}`,
            title: `Tender Sub: ${(t.title || 'Unnamed Tender').slice(0, 15)}...`,
      ...
      assignments.forEach(a => {
        if (a.deadline) {
          allEvents.push({
            id: `a_${a.id}`,
            title: `Project: ${(a.title || 'Unnamed Project').slice(0, 15)}...`,
  ...
  // Render Month View (Existing Grid Logic)
    ...
    return (
      <div className="overflow-x-auto">
        <div className="min-w-[800px]">
          ...
          <div className={`grid grid-cols-7 ${totalCells > 35 ? 'grid-rows-6' : 'grid-rows-5'} min-h-[600px]`}>
            {calendarCells.map((cell, i) => {
              ...
              return (
                <div
                  ...
                  className="border-r border-b border-slate-50 p-2 md:p-3 relative group hover:bg-slate-50/50 transition-colors min-h-[120px] cursor-pointer"
                >
                  <div className="flex justify-between items-start">
                    <span className={`w-6 h-6 flex items-center justify-center rounded-full text-[11px] font-black ${cell.isPrevMonth || cell.isNextMonth ? 'text-slate-200' : 'text-slate-400'} ${isToday ? 'bg-blue-600 text-white' : ''}`}>
                      {cell.dayNum}
                    </span>
                  </div>
  ...
  const renderWeekView = () => {
    ...
    return (
      <div className="overflow-x-auto min-h-[500px]">
        <div className="min-w-[1000px] grid grid-cols-7 border-b border-slate-50 bg-slate-50/30 h-full">
          {weekDays.map((wd, i) => {
            ...
            return (
              <div key={i} className="border-r border-slate-50 flex flex-col min-h-[500px]">
  ...
  const renderDayView = () => {
    ...
    return (
      <div className="flex flex-col space-y-6 p-6 md:p-8 bg-slate-50/30">
          ...
          {weekDays.map((wd, i) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center flex-1 py-2 rounded-xl transition-all hover:bg-slate-50 relative group"
              >
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{days[wd.getDay()]}</span>
                <span className={`w-8 h-8 flex items-center justify-center rounded-full text-xs font-black transition-all ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col">
            ...
            <div ref={timelineRef} className="relative h-[600px] overflow-y-auto border border-slate-100 rounded-2xl bg-slate-50/10 custom-scrollbar">
              <div className="relative min-w-[500px]" style={{ height: '1440px' }}>
                ...
                {hours.map(h => (
                  <div key={h} className="absolute left-0 right-0 flex border-b border-slate-100" style={{ top: `${h * 60}px`, height: '60px' }}>
                    <span className="w-[70px] text-right pr-4 pt-1 text-[10px] font-black text-slate-400 uppercase tracking-wider select-none shrink-0">
                      {formatHourLabel(h)}
                    </span>
                ...
                {dayEvents.map(e => {
                  ...
                  return (
                    <div
                      ...
                      className={`absolute left-[85px] right-4 p-3 rounded-xl cursor-pointer transition-all border flex flex-col justify-between shadow-sm group hover:scale-[1.01] ${
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span className="text-xs font-black text-slate-800 truncate" style={{ color: isSelected ? e.color : undefined }}>{e.title}</span>
                        <span className="text-[9px] font-bold text-slate-400 bg-white/80 px-1.5 py-0.5 rounded border border-slate-100">
          ...
          {/* Right Panel: Selected Event Details Sidebar */}
            ...
            <div className="bg-slate-50/50 p-6 rounded-2xl border border-slate-100 space-y-6">
              {activeEvent ? (
                <div className="space-y-6">
                  ...
                  {/* Event Title */}
                  <div>
                    <h3 className="text-xl font-black text-slate-800 tracking-tight leading-snug">{activeEvent.title}</h3>
                  </div>
  ...
  const legend = [
    { label: 'Project Deadlines', color: '#3b82f6' },
    { label: 'Tender Deadlines', color: '#ef4444' },
    ...
    <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-[#fbfcfd]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className={`${view === 'Day' ? 'lg:col-span-12' : 'lg:col-span-9'} bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col`}>
          <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/10 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-lg font-black text-slate-800 tracking-tight text-center min-w-[200px]">{getHeaderTitle()}</h2>
              ...
              <div className="flex bg-slate-50 p-1 rounded-xl">
                {['Year', 'Month', 'Week', 'Day'].map((v) => (
                  <button
                    key={v}
                    onClick={() => setView(v)}
                    className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${view === v ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {v}
                  </button>
                ))}
              </div>
              <button onClick={goToday} className="px-5 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-lg text-[10px] font-black transition-colors">Today</button>
            </div>
          </div>
          ...
          <div className="p-6 md:p-8 border-t border-slate-50 flex flex-wrap gap-6 justify-start shrink-0">
            {legend.map((item, i) => (
        ...
        {view !== 'Day' && (
          <div className="lg:col-span-3 space-y-8">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-8">
              <div>
                <h3 className="text-xs font-black text-slate-900 tracking-[0.2em] uppercase mb-4">Upcoming Events</h3>
                <div className="space-y-4">
                  <div className="space-y-3">
                    {upcomingEvents.length === 0 ? (
                      <p className="text-[10px] font-bold text-slate-400 italic">No upcoming events.</p>
                    ) : (
                      upcomingEvents.map(e => (
                        <div key={e.id} className="flex justify-between items-center text-[10px] font-bold text-slate-500">
                          <span className="truncate max-w-[140px] text-slate-700" title={e.title}>{e.title}</span>
                          <span className="text-slate-400 shrink-0" style={{ color: e.color }}>{e.date.toLocaleDateString()}</span>
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="pt-8 border-t border-slate-50">
                <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest mb-4">Quick Notes</h3>
            ...
            <form onSubmit={handleAddReminder} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder="E.g., Client Alpha Review"
                />
              </div>
