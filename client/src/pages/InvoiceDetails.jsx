...
const InvoiceDetails = ({ invoiceId, onBack }) => {
  ...
  const getAttachmentUrl = (inv) => {
    let atts = inv.attachments || [];
    if (typeof atts === 'string') {
      try { atts = JSON.parse(atts); } catch (e) { atts = []; }
    }
    if (Array.isArray(atts) && atts.length > 0) {
      return atts[0].url || atts[0];
    }
    return null;
  };

  const attachmentUrl = getAttachmentUrl(invoice);

  const renderUploadedInvoice = (url) => {
    if (!url) return null;
    const lowerUrl = url.toLowerCase();
    const isImage = lowerUrl.endsWith('.jpg') || lowerUrl.endsWith('.jpeg') || lowerUrl.endsWith('.png') || lowerUrl.endsWith('.gif') || lowerUrl.endsWith('.webp') || lowerUrl.includes('image');
    
    if (isImage) {
      return (
        <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden p-4 sm:p-8 flex justify-center items-center print:border-none print:shadow-none print:p-0">
          <img src={url} className="max-w-full h-auto object-contain max-h-[1200px]" alt="Uploaded Invoice" />
        </div>
      );
    }
    
    return (
      <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden h-[80vh] print:border-none print:shadow-none print:h-screen print:w-screen">
        <iframe src={url} className="w-full h-full border-none" title="Uploaded Invoice PDF" />
      </div>
    );
  };
  ...
  return (
    <div className="p-4 sm:p-8 animate-in fade-in duration-500 print:p-0 print:bg-white text-left">
      ...
      {attachmentUrl ? (
        renderUploadedInvoice(attachmentUrl)
      ) : (
