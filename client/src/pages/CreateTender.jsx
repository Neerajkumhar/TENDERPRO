              ...
              <div className="card p-10 bg-white border-none shadow-2xl shadow-slate-200/40">
                ...
                <div className="grid grid-cols-3 gap-8 text-left">
                  <div className="space-y-2">
                    ...
                    <select value={formData.teamAssignments?.managerId} onChange={(e) => setFormData({ ...formData, teamAssignments: { ...formData.teamAssignments, managerId: e.target.value } })} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-[2rem] text-sm font-bold text-slate-700 outline-none appearance-none cursor-pointer focus:border-blue-500 focus:bg-white transition-all shadow-sm"><option value="">Select manager</option>{members.map(member => (<option key={member.id} value={member.id}>{member.name} ({member.role} - {member.email})</option>))}</select>
                  </div>
                  <div className="space-y-2">
