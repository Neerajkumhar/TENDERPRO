    ...
    <div className="p-8 max-w-[1600px] mx-auto min-h-screen bg-[#f8fafc]">
      <div className="bg-white px-8 py-6 rounded-[2rem] shadow-2xl shadow-slate-200/30 border border-slate-100 mb-8">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl xl:text-5xl font-black uppercase tracking-tight text-slate-900">Delivery Challans</h1>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-[420px]">
              ...
              <input
                ...
                placeholder="Search challans, clients..."
      ...
      {receiptOpen && selected && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center px-2 py-4">
          ...
          <div id="delivery-receipt" className="relative bg-white w-full max-w-[1100px] shadow-2xl overflow-hidden border border-slate-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative z-0 p-8 print:p-8 space-y-6 print:space-y-4 print:border-2 print:border-slate-800 print:h-[calc(100vh-10mm)] print:rounded-sm print:flex print:flex-col">
              {/* WATERMARK */}
              ...
              <div className="flex items-start justify-between gap-6">
                ...
                <div className="flex-1 text-right">
                  <div className="text-xl font-black text-blue-900 tracking-tight">DELIVERY CHALLAN</div>
                  <table className="mt-4 ml-auto text-sm text-left">
                    <tbody>
                      <tr>
                        <td className="font-bold text-slate-700 pr-6 text-left py-1">Challan No:</td>
                        <td className="font-medium text-slate-600 py-1">{selected.id}</td>
                      </tr>
                      <tr>
                        <td className="font-bold text-slate-700 pr-6 text-left py-1">Date:</td>
                        <td className="font-medium text-slate-600 py-1">{selected.dispatchDate || ''}</td>
                      </tr>
                      <tr>
                        <td className="font-bold text-slate-700 pr-6 text-left py-1">Status:</td>
                        <td className="py-1">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${selected.status === 'Paid' ? 'bg-emerald-100 text-emerald-700' : selected.status === 'Overdue' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>
                            {selected.status}
                          </span>
                        </td>
                      </tr>
              ...
              <div className="grid grid-cols-2 gap-8 py-6 border-y border-slate-100">
                <div className="space-y-3">
                  ...
                  <div className="flex">
                    ...
                    <span className="text-sm font-bold text-slate-800 flex-1">: {selected.shippingAddress || ''}</span>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex">
                    <span className="w-32 flex-shrink-0 text-sm text-slate-500 font-medium">Invoice Ref</span>
                    <span className="text-sm font-bold text-slate-800 flex-1">: {selected.invoiceRef || 'INV/2025/0456'}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 flex-shrink-0 text-sm text-slate-500 font-medium">PO / Ref No.</span>
                    <span className="text-sm font-bold text-slate-800 flex-1">: {selected.poRef || 'PO/2025/01234'}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 flex-shrink-0 text-sm text-slate-500 font-medium">PO Date</span>
                    <span className="text-sm font-bold text-slate-800 flex-1">: {selected.poDate || '12/05/2025'}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 flex-shrink-0 text-sm text-slate-500 font-medium">Delivery Reason</span>
                    <span className="text-sm font-bold text-slate-800 flex-1">: Material Supply against PO</span>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-2xl shadow-slate-200/30 overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Delivery Challans Ledger</h2>
            <p className="text-sm text-slate-500 mt-1">Monitor dispatches, receipts, shipping mode, and signed documents.</p>
          </div>
          ...
          <table className="w-full min-w-[1100px] text-left">
            <thead className="bg-slate-50 text-[10px] font-black uppercase tracking-[0.35em] text-slate-400 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-5">LR &amp; Gate Pass</th>
                <th className="px-6 py-5">Dispatch Date</th>
                <th className="px-6 py-5">Material Value</th>
                <th className="px-6 py-5">E-way Bill</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredChallans.length > 0 ? filteredChallans.map((challan) => (
                <tr key={challan.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-5 text-slate-700">{challan.transporter}</td>
                  <td className="px-6 py-5 text-slate-700">{challan.lrGatePass}</td>
                  <td className="px-6 py-5 text-slate-700">{challan.dispatchDate}</td>
                  <td className="px-6 py-5 font-black text-slate-900">₹{challan.materialValue.toLocaleString()}</td>
                  <td className="px-6 py-5 text-slate-700">{challan.eWayBill}</td>
                  <td className="px-6 py-5">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] ${statusClasses[challan.status] || 'bg-slate-400 text-white'}`}>
                      {challan.status}
                    </span>
                  </td>
                ...
                <tr>
                  <td colSpan="9" className="px-6 py-10 text-center text-slate-400 font-bold italic">No delivery challans found</td>
                </tr>
              )}
            </tbody>
          </table>
