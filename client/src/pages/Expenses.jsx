...
const Expenses = ({ onViewExpense, user }) => {
  ...
  const handleExportReport = ({ format, startDate, endDate }) => {
    ...
    if (format === 'xlsx') {
      ...
      const doc = new jsPDF();
      autoTable(doc, {
        head: [["ID", "Dept", "Category", "Vendor", "Date", "Amount", "Status"]],
        body: exportData.map(item => [item.id, item.department, item.category, item.vendor, item.date, item.amount.toLocaleString(), item.status]),
      });
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 sm:mb-10 gap-6">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-center gap-3 w-full xl:w-auto">
          <div className="relative w-full sm:flex-1 sm:min-w-[200px] xl:w-80">
            ...
            <input type="text" placeholder="Search expenses..." className="w-full pl-12 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold focus:outline-none focus:border-blue-400 transition-all shadow-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
          
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none">
              <button onClick={() => setShowFilterDropdown(!showFilterDropdown)} className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest transition-all shadow-sm ${showFilterDropdown ? 'border-blue-400 text-blue-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                <Filter size={16} className="text-blue-500" />
                <span>Filters</span>
              </button>
              {showFilterDropdown && (
                <div className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-full sm:w-72 bg-white border border-slate-100 shadow-2xl rounded-2xl p-4 sm:p-5 z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setCategoryFilter('ALL'); setStatusFilter('ALL'); }} className="flex-1 py-2 bg-slate-100 text-slate-600 rounded-xl text-[9px] font-black uppercase tracking-widest">Reset</button>
                      <button onClick={() => setShowFilterDropdown(false)} className="flex-[2] py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-md">Apply</button>
                    </div>
                  </div>
                </div>
              )}
          ...
          <table className="w-full text-left min-w-[850px]">
            <thead>
              <tr className="bg-slate-50/50 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                ...
                <th className="px-6 sm:px-8 py-4 sm:py-5">Dept</th>
              ...
              {filteredExpenses.length > 0 ? (
                filteredExpenses.map((expense, index) => (
                  <tr key={expense.id || index} className="transition-all hover:bg-slate-50 group">
                    ...
                    <td className="px-6 sm:px-8 py-4 sm:py-6 text-xs sm:text-sm font-bold text-slate-400 uppercase">{expense.department || '-'}</td>
            ...
            <div className="p-6 sm:p-10 overflow-y-auto custom-scrollbar">
              ...
              <form onSubmit={handleSaveExpense} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Department</label><select className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold appearance-none cursor-pointer" value={formData.department} onChange={(e) => setFormData({...formData, department: e.target.value})}>{departments.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
