  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight uppercase italic">Team Management</h1>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95"
          >
            <UserPlus size={18} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          <div key={i} className="card p-3 sm:p-4 bg-white border-none shadow-lg shadow-slate-200/50 hover:shadow-xl hover:-translate-y-1 transition-all group cursor-pointer overflow-hidden relative">
            ...
            <h3 className="text-base sm:text-xl font-black text-slate-900 mt-1 tracking-tight">{stat.value}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-12 gap-4 sm:gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6 sm:space-y-8">
          <div className="card bg-white border-none shadow-xl shadow-slate-200/40 overflow-hidden rounded-[1.5rem] sm:rounded-[2rem]">
            <div className="p-4 sm:p-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/30">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 w-full">
                <h3 className="font-black text-slate-900 text-base sm:text-lg tracking-tight uppercase italic shrink-0">Team Directory</h3>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    ...
                    <th className="px-6 py-4 hidden md:table-cell">Workload</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="p-1.5 sm:p-2 text-slate-300 hover:text-rose-600 rounded-lg hover:bg-rose-50 transition-all"
                        >
          ...
          <div className="card bg-white border-none shadow-xl shadow-slate-200/40 overflow-hidden rounded-[1.5rem] sm:rounded-[2rem]">
            <div className="p-4 sm:p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-black text-slate-900 text-base sm:text-lg tracking-tight uppercase italic">Departments</h3>
              </div>
              <button 
                onClick={() => setIsCreateDeptOpen(true)}
                className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 active:scale-95"
              >
                <Plus size={14} />
                <span>New Dept</span>
              </button>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    ...
                    <th className="px-6 py-4 hidden sm:table-cell">Theme</th>
                    <th className="px-6 py-4 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {departments.map((dept) => (
                    <tr key={dept.id} className="hover:bg-indigo-50/30 transition-all group">
                      <td className="px-6 py-4">
                        ...
                        <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold truncate max-w-[150px]">{dept.description || 'No description'}</p>
                      </td>
                        ...
                        <button 
                          onClick={(e) => {
                            ...
                            if(window.confirm('Delete this department?')) {
                              fetch(`/api/departments/${dept.id}`, { method: 'DELETE' }).then(() => {
                                fetchDepartments();
                              });
                            }
                          }}
                          className="p-1.5 sm:p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all"
                        >
                          <Plus size={16} className="rotate-45" />
                        </button>
                      </td>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6 sm:space-y-8">
          ...
          <div className="card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 rounded-[2rem]">
            <h3 className="font-black text-slate-900 text-base sm:text-lg tracking-tight mb-6 uppercase italic">Distribution</h3>
              ...
              {departments.map((dept, i) => (
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] sm:text-xs font-bold text-slate-600 uppercase tracking-tight">{dept.name}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">{dept.memberCount || 0}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Performance Leaders */}
          <div className="card p-5 sm:p-6 bg-white border-none shadow-xl shadow-slate-200/40 rounded-[2rem]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black text-slate-900 text-base sm:text-lg tracking-tight uppercase italic">Performers</h3>
              ...
              {teamMembers.slice(0, 3).map((member, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100 group hover:border-blue-200 transition-all cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      {member.image ? (
                        <img src={member.image || null} className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl shadow-sm object-cover" alt="" />
                      ) : (
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400">
                          <User size={18} />
                        </div>
                      )}
                    ...
                    <div className="min-w-0">
                      <p className="text-[11px] sm:text-xs font-black text-slate-900 truncate">{member.name}</p>
                      <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold truncate">{getDeptName(member.departmentId)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-black text-blue-600">4.5</p>
          ...
          <div className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">New Dept.</h2>
      ...
      {isAddMemberOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 h-full sm:h-auto overflow-y-auto">
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Add Member</h2>
                  <p className="text-xs text-slate-500 font-medium italic">Create system access for new joiner</p>
                </div>
              </div>
            ...
            <form onSubmit={handleAddMember} className="p-6 sm:p-8 space-y-5">
