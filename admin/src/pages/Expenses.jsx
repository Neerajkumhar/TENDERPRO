...
const Expenses = ({ onViewExpense }) => {
  ...
  useEffect(() => {
    ...
    if (parsedBudgets && parsedBudgets.length > 0) {
      ...
      setFormData(prev => ({ 
        ...prev, 
        department: parsedBudgets[0].department || 'OPERATIONS',
        category: budgetNames[0] 
      }));
    }
  }, []);
  ...
  const handleSaveExpense = async (e) => {
    ...
    setFormData({
      department: budgetList.length > 0 ? (budgetList[0].department || 'OPERATIONS') : 'OPERATIONS',
  ...
  const startEditExpense = (expense) => {
    ...
    // Find the department for this category if possible
    const expenseBudget = budgetList.find(b => b.name === expense.category);
    
    setFormData({
      department: expenseBudget ? expenseBudget.department : 'OPERATIONS',
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 bg-[#f8fafc] min-h-screen relative">
      {/* Header Row - Matching Reference Image */}
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-10 gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase">EXPENSES</h1>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <div className="relative">
            ...
            {showFilterDropdown && (
              <div className="absolute right-0 mt-2 w-72 bg-white border border-slate-100 shadow-2xl rounded-2xl p-5 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-6 mb-12">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100 flex flex-col justify-center">
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/30 border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-white">
          <h2 className="text-lg font-black text-slate-800 tracking-tight italic uppercase">Expense Ledger</h2>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                <th className="px-8 py-5">Expense ID</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredExpenses.length > 0 ? (
                  ...
                  <tr key={expense.id || index} className="transition-all hover:bg-slate-50 group">
                    <td className="px-8 py-6">
                      <button 
                        ...
                        className="text-sm font-black text-blue-600 hover:text-blue-800 tracking-tight hover:underline focus:outline-none"
                      >
                        {expense.id}
                      </button>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2">
                         ...
                         <span className="text-sm font-black text-slate-800 tracking-tight uppercase">{expense.category}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-sm font-bold text-slate-500 uppercase tracking-tighter">{expense.vendor}</td>
                    <td className="px-8 py-6 text-sm font-bold text-slate-400">{expense.date}</td>
                    <td className="px-8 py-6 text-sm font-bold text-slate-500">{expense.description}</td>
                    <td className="px-8 py-6 text-sm font-black text-slate-900 text-right">${Number(expense.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                ...
                <tr>
                  <td colSpan="8" className="px-8 py-10 text-center text-slate-400 font-bold italic">
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        const filteredCats = budgetList.filter(b => (b.department || 'OPERATIONS') === newDept).map(b => b.name);
                        setFormData({...formData, department: newDept, category: filteredCats.length > 0 ? filteredCats[0] : ''});
                      }}
                    >
                      {[...new Set(budgetList.map(b => b.department || 'OPERATIONS'))].map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Budget Category</label>
                    <select 
                      className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer"
                      value={formData.category}
                      onChange={(e) => setFormData({...formData, category: e.target.value})}
                    >
                      {budgetList.filter(b => (b.department || 'OPERATIONS') === formData.department).map(b => b.name).map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                </div>
