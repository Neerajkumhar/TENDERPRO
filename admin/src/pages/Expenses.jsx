...
const Expenses = ({ onViewExpense }) => {
  ...
  const stats = [
    ...
    { label: 'TOTAL AMOUNT', value: `₹${totalAmountSum.toLocaleString('en-IN')}`, color: 'text-slate-900' },
  ];
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5 text-left">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button 
            onClick={() => { 
              setEditingExpense(null); 
              setFormData({ category: 'OFFICE', vendor: '', date: new Date().toISOString().split('T')[0], description: '', amount: '', status: 'PENDING', document: null }); 
              setIsModalOpen(true); 
            }} 
              ...
              {filteredExpenses.length > 0 ? (
                filteredExpenses.map((expense, index) => (
                  <tr key={expense.id || index} className="transition-all hover:bg-slate-50/70 group">
                    ...
                    <td className="px-3.5 py-2 text-[10.5px] font-medium text-slate-500 truncate max-w-[160px]">{expense.description}</td>
