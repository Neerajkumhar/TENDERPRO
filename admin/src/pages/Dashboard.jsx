...
const Dashboard = ({ user, members = [], assignments = [], onProjectClick }) => {
  ...
  // Transform members for workload view if assignments are available
  const teamWorkloadData = members.slice(0, 5).map(member => {
    const memberTasks = assignments.filter(a => a.userId === member.id).length;
    const workload = Math.min(Math.floor((memberTasks / 10) * 100), 100) || Math.floor(Math.random() * 40) + 30;
    
    return {
      id: member.id,
      name: member.name,
      tasks: memberTasks || Math.floor(Math.random() * 8) + 2,
      workload: workload,
      image: member.image,
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 animate-in fade-in duration-700 max-w-[1600px] mx-auto overflow-x-hidden">
      {/* Welcome Header */}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto">
          {(user?.role === 'Admin' || user?.role?.includes('Manager')) && (
            <button 
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-black hover:bg-amber-600 transition-all shadow-lg shadow-amber-200 active:scale-95 whitespace-nowrap"
            >
              <Coffee size={18} />
          ...
          <div className="relative flex-1 sm:flex-none" ref={datePickerRef}>
            <button 
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 hover:border-blue-300 transition-all shadow-sm active:scale-95 whitespace-nowrap"
            >
            ...
            {showDatePicker && (
              <div className="absolute right-0 sm:right-auto sm:left-0 xl:right-0 xl:left-auto mt-2 p-4 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 w-64 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 sm:mb-10 text-left">
              <div>
                <h3 className="font-black text-slate-900 text-lg sm:text-xl tracking-tight uppercase tracking-wider">Tender Overview</h3>
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Last 6 months performance</p>
              </div>
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 bg-blue-500 rounded-full shadow-lg shadow-blue-200"></div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Won</span>
                  </div>
                </div>
                <select className="text-[10px] font-black uppercase tracking-widest bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 outline-none shadow-sm cursor-pointer hover:bg-white hover:border-blue-300 transition-all">
            ...
            <div className="h-[280px] sm:h-[320px] w-full">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <LineChart data={chartData}>
                  ...
                  />
                  <Line type="monotone" dataKey="created" stroke="#3b82f6" strokeWidth={4} dot={{r: 4, fill: '#3b82f6', strokeWidth: 2, stroke: '#fff'}} activeDot={{r: 6}} />
                  <Line type="monotone" dataKey="won" stroke="#10b981" strokeWidth={2.5} strokeDasharray="5 5" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="card p-6 sm:p-8 bg-white border-none shadow-xl shadow-slate-200/40">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 text-left">
              <div>
                <h3 className="font-black text-slate-900 text-lg sm:text-xl tracking-tight uppercase tracking-wider">Financial Snapshot</h3>
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Revenue & payment flow</p>
              </div>
              <select className="text-[10px] font-black uppercase tracking-widest bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 outline-none shadow-sm cursor-pointer hover:bg-white hover:border-blue-300 transition-all w-full sm:w-auto">
                <option>This Month</option>
              </select>
            </div>
            <div className="flex flex-col md:flex-row items-center gap-8 lg:gap-10 h-auto md:h-[320px]">
              <div className="w-full md:w-1/2 h-[240px] sm:h-[280px] md:h-full relative group shrink-0">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <PieChart>
                    <Pie
                      data={financialData}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none group-hover:scale-110 transition-transform duration-500">
                  <span className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tighter italic">₹245k</span>
                  ...
                  <div key={i} className="group cursor-pointer">
                    <div className="flex justify-between items-center mb-1.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[10px] font-black text-slate-500 group-hover:text-slate-900 transition-colors uppercase tracking-widest">{item.name}</span>
                      </div>
                      <span className="text-xs font-black text-slate-900 tracking-tight italic">₹{(item.value / 1000).toFixed(1)}k</span>
                    </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
          ...
          <div className="card col-span-1 xl:col-span-2 overflow-hidden bg-white border-none shadow-xl shadow-slate-200/40">
            <div className="p-6 sm:p-8 border-b border-slate-50 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-slate-50/20 text-left">
              <div>
                <h3 className="font-black text-slate-900 text-lg sm:text-xl tracking-tight uppercase tracking-wider">Recent Tenders</h3>
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Application monitoring log</p>
              </div>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left min-w-[700px]">
                <thead>
                  <tr className="bg-slate-50/30 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    <th className="px-8 py-5">ID</th>
                    <th className="px-8 py-5">Tender Name</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {tenderTable.map((tender, i) => (
                    <tr key={i} className="hover:bg-blue-50/20 transition-all cursor-pointer group">
                      <td className="px-8 py-6 text-xs font-bold text-slate-400">#{tender.id.split('-').pop()}</td>
                      <td className="px-8 py-6">
                        <p className="text-sm font-black text-slate-800 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{tender.name}</p>
                        <p className="text-[9px] text-slate-400 font-black mt-1 uppercase tracking-widest">Due: {tender.date}</p>
                      </td>
                      <td className="px-8 py-6 text-xs font-bold text-slate-600 uppercase">{tender.client}</td>
                      <td className="px-8 py-6 text-sm font-black text-slate-900 text-right italic">{tender.value}</td>
                      <td className="px-8 py-6">
                        <div className={`mx-auto w-fit px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm
                          ${tender.status === 'Active' ? 'bg-blue-600 text-white shadow-blue-100' : 
                            tender.status === 'Pending' ? 'bg-amber-500 text-white shadow-amber-100' : 
                            tender.status === 'Submitted' ? 'bg-emerald-500 text-white shadow-emerald-100' : 
                            'bg-rose-50 text-white shadow-rose-100'}`}>
                          {tender.status}
                        </div>
                      </td>
                    </tr>
            ...
            <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
              {teamWorkloadData.map((member, i) => (
                <div key={i} className="group cursor-pointer p-4 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                  <div className="flex items-center gap-4 mb-3">
                    <div className="relative shrink-0">
                      {member.image ? (
                        ...
                        <div className="w-11 h-11 rounded-2xl bg-slate-100 border-2 border-white shadow-sm flex items-center justify-center text-slate-400 group-hover:scale-105 transition-transform">
                          <User size={20} />
                        </div>
                      )}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{member.tasks} Tasks Assigned</p>
                    </div>
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ease-out ${member.workload > 75 ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)]' : member.workload > 50 ? 'bg-blue-600' : 'bg-emerald-500'}`} 
      ...
      {showLeaveModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 text-left">
          ...
          <div className="bg-white w-full max-w-4xl max-h-[90vh] sm:max-h-[85vh] rounded-3xl sm:rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col">
            ...
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-amber-500 text-white rounded-2xl shadow-lg shadow-amber-200 shrink-0">
                  <Coffee size={24} className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <div className="min-w-0">
                  <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight uppercase truncate">Team Leave Requests</h2>
                  <p className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-widest mt-1 opacity-70 truncate">Manage department time-off apps</p>
                </div>
              </div>
              <button 
                onClick={() => setShowLeaveModal(false)}
                className="p-2 sm:p-3 hover:bg-white rounded-full text-slate-400 hover:text-slate-900 transition-all border border-transparent hover:border-slate-100 shadow-sm shrink-0"
              >
                <X size={24} />
              </button>
            </div>
                  ...
                  {leaveRequests.map((request) => (
                    <div key={request.id} className="bg-white border border-slate-100 rounded-3xl p-5 sm:p-6 shadow-xl shadow-slate-200/40 hover:shadow-2xl hover:border-blue-200 transition-all group relative overflow-hidden">
                      ...
                      <div className="flex items-start gap-4 mb-6 text-left">
                        <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-black text-xl border-2 border-white shadow-sm overflow-hidden shrink-0">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100/50 text-left">
                        ...
                        <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100/50 h-24 sm:h-28 text-left">
                          <div className="p-2 bg-white rounded-xl text-amber-500 shadow-sm shrink-0">
                            <AlertCircle size={16} />
                          </div>
                          <div className="flex-1 min-w-0 overflow-y-auto pr-1 custom-scrollbar text-left">
                            ...
                            <p className="text-[11px] font-bold text-slate-600 italic">
                              <span className="text-slate-900 font-black not-italic uppercase tracking-tighter mr-1">{request.leaveType}:</span>
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-center gap-4 sm:px-10 shrink-0">
               <div className="flex items-center gap-4">
                  <div className="flex -space-x-3">
                     {[1,2,3].map(i => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[10px] font-black text-slate-400 shadow-sm overflow-hidden shrink-0">
                           <User size={14} />
                        </div>
                     ))}
                  </div>
               ...
               <button 
                onClick={() => setShowLeaveModal(false)}
                className="w-full sm:w-auto px-10 py-3.5 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl active:scale-95"
               >
                 Close Panel
               </button>
            </div>
