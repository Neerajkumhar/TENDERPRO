  ...
  return (
    <div className="p-10 max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 bg-[#f8fafc] min-h-screen">
      ...
      <div className="flex justify-between items-start mb-10">
        <div>
          <h1 className="text-3xl font-black text-[#1e293b] tracking-tight">Projects</h1>
          <p className="text-slate-500 mt-1 font-semibold">Manage and track all your active projects</p>
        </div>
        <button 
          onClick={onCreateProject}
          className="flex items-center gap-2 px-6 py-3 bg-[#1e293b] text-white rounded-xl text-sm font-black transition-all hover:bg-slate-800 shadow-lg shadow-slate-200 active:scale-95"
        >
          <Plus size={20} />
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-6 border-b border-slate-200 pb-1">
        <div className="flex">
           <button className="px-4 py-4 text-[11px] font-black uppercase tracking-widest text-[#1e293b] border-b-2 border-[#1e293b] -mb-[2px]">
             All Projects
           </button>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto pb-4 md:pb-0">
           ...
           <button 
             ...
             className={`p-2.5 border rounded-xl transition-all shadow-sm relative ${
           ...
           <button 
             ...
             className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-slate-600 hover:border-slate-300 transition-all shadow-sm active:scale-95"
      ...
      {showDateFilter && (
        <div className="mb-6 p-5 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-100/50 flex flex-col md:flex-row items-end gap-5 animate-in slide-in-from-top-4 duration-300">
           <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
              <div>
                 <label className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-2">Projects Created From (Start Date)</label>
                 <input 
                   type="date"
              ...
              <div>
                 <label className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-2">Projects Created To (End Date)</label>
                 <input 
                   type="date"
                   value={filterEndDate}
           ...
           <div className="flex gap-2 w-full md:w-auto">
              <button 
                ...
                className="flex-1 md:flex-none px-5 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm font-bold hover:bg-slate-50 hover:text-slate-700 transition-all active:scale-95"
              >
                 Reset
              </button>
              <button 
                onClick={() => setShowDateFilter(false)}
                className="flex-1 md:flex-none px-6 py-2.5 bg-[#1e293b] text-white rounded-xl text-sm font-black hover:bg-slate-800 transition-all active:scale-95 shadow-md shadow-slate-200"
              >
                 Close Filter
              </button>
           </div>
      ...
      <div className="bg-white rounded-[1.5rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-visible">
        <table className="w-full text-left">
          <thead>
            <tr className="bg-white text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
              ...
              <th className="px-8 py-6">Tender</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {myProjects.length > 0 ? myProjects.map((item) => (
              ...
              >
                ...
                <td className="px-8 py-6 text-sm font-bold text-slate-500">{item.tender?.title || 'N/A'}</td>
                <td className="px-8 py-6 text-sm font-bold text-slate-500">{item.tender?.client?.name || 'N/A'}</td>
                <td className="px-8 py-6 text-sm font-bold text-slate-500">{new Date(item.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}</td>
                <td className="px-8 py-6 text-sm font-bold text-slate-500">{item.deadline ? new Date(item.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'N/A'}</td>
                <td className="px-8 py-6 text-sm font-black text-slate-900">₹{parseFloat(item.tender?.budget || 0).toLocaleString()}</td>
                <td className="px-8 py-6">
                ...
                <td className="px-8 py-6">
                   <span className="px-3 py-1 bg-slate-100 rounded-lg text-xs text-slate-600 font-extrabold tracking-tight">
              ...
              <tr>
                <td colSpan="9" className="px-8 py-20 text-center">
                   <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-[10px] text-slate-300 uppercase font-black tracking-widest">Assigned to: {user.name}</p>
                   </div>
                </td>
              </tr>
            )}
