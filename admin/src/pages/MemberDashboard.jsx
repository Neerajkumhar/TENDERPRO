      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            ...
            <button className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black text-slate-600 uppercase">
              <span>This Week</span>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-full h-[220px]">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-4xl font-black text-slate-900 tracking-tighter">{tasks.length || 28}</span>
        ...
        <div className="lg:col-span-5 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <button className="w-full py-5 rounded-[2rem] border-2 border-dashed border-slate-100 flex items-center justify-center gap-3 text-slate-300 hover:border-blue-200 hover:text-blue-500 hover:bg-blue-50/20 transition-all group">
               <Plus size={18} />
               <span className="text-[10px] font-black uppercase tracking-widest">New Daily Task</span>
            </button>
          </div>
        </div>

        {/* Right Task Category Sidebar */}
        <div className="lg:col-span-3 bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col h-full overflow-hidden">
          <div className="flex items-center gap-4 border-b border-slate-50 pb-2 overflow-x-auto scrollbar-hide">
            {['ALL', 'TODAY', 'OVERDUE'].map((tab) => (
              <button 
                key={tab}
                className={`text-[9px] font-black uppercase tracking-widest px-3 py-2 rounded-xl transition-all whitespace-nowrap
                  ${tab === 'ALL' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {tab}
              </button>
            ))}
          ...
          <div className="mt-6 flex-1 space-y-4">
            {tasks.filter(t => String(t.id).startsWith('s')).map((task) => (
              <div 
                key={task.id} 
                id={`task-${task.id}`}
                draggable
                onDragStart={(e) => handleDragStart(e, task.id)}
                onDragEnd={handleDragEnd}
                className="p-5 bg-slate-50/50 border border-slate-50 rounded-3xl hover:border-slate-200 hover:bg-white hover:shadow-xl transition-all duration-300 cursor-grab active:cursor-grabbing group relative overflow-hidden"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-1.5 rounded-lg bg-white border border-slate-100 text-slate-300 shadow-sm group-hover:text-blue-600 transition-all">
                    <AlertCircle size={14} />
                  </div>
                  <h4 className="text-[12px] font-black text-slate-800 tracking-tight leading-tight line-clamp-2 uppercase">{task.title}</h4>
                </div>
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[9px] font-bold text-slate-400 tracking-tight italic">{task.time}</p>
                    <p className={`text-[8px] font-black uppercase tracking-widest ${task.due === 'OVERDUE' ? 'text-rose-500' : 'text-slate-500'}`}>
                      {task.due || (task.status === 'Completed' ? 'DONE' : 'PENDING')}
                    </p>
                  </div>
                  <div className="flex -space-x-1.5">
                    {[1, 2].map(i => (
                       <div key={i} className="w-5 h-5 rounded-full bg-slate-100 border border-white text-[8px] font-black flex items-center justify-center text-slate-400">
                          {i === 1 ? 'MK' : '+1'}
                       </div>
                    ))}
                  </div>
