...
const TenderDetails = ({ tenderId, onBack, onEdit, onDelete, onProjectClick, user = {}, members = [] }) => {
  ...
  const getMemberName = (id) => {
    ...
    return match ? `${match.name} (${match.role || 'Member'})` : 'Unassigned';
  };
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
          {canSubmitCompletion && (
            <button 
              ...
              className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[10px] font-bold shadow-2xs hover:bg-blue-700 transition-all active:scale-95 uppercase tracking-wider flex items-center gap-1.5"
            >
              <Flag size={13} />
